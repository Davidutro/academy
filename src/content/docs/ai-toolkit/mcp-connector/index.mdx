...
Note: currently this setting is not available in the mobile app, but can be configured in Claude.ai / Claude Desktop and then used in the mobile app.

1. Open Claude and navigate to the Customize->Connectors section.
2. Click on the "+" button to add a new connector and select "Add Custom Connector".
3. Specify the name as **Santiment** and in `Remote MCP server URL` paste
...
5. Log in (or create a free account at [app.santiment.net](https://app.santiment.net)) and approve the connection.
...
Note: currently this setting is not available in the mobile app, but can be configured in chatgpt.com and then used in the mobile app.

1. Open chatgpt.com and navigate to the Settings->Apps->Advanced Settings section.
2. Turn on the "Developer Mode" switch and click on the "Create app" button.
3. Specify the name as **Santiment**, MCP Server URL as `https://api.santiment.net/mcp`, Authentication as `OAuth`, confirm security risk warning and click Create.
4. You'll be redirected to Santiment to authorize access via OAuth.
5. Log in (or create a free account at [app.santiment.net](https://app.santiment.net)) and approve the connection.
6. Open new chat, click "+" button and select "Santiment" from the list of connectors/apps. Start asking questions about any token, metric, or market condition.

## Authentication

The connector uses **OAuth 2.0 authorization code flow** with PKCE. When you first connect, you'll be prompted to log in to your Santiment account and authorize your AI Tool to access data on your behalf.
