...
Sentiment Ratio compares [Labeled Social Volume](/metrics/social-volume-labeled) counts to show what share of documents fall into each sentiment bucket — positive, neutral, or negative.
...
Each source is available at three frequencies: the plain metric name is computed on [five-minute intervals](/metrics/details/frequency#five-minute-frequency), the `_1h` suffix on [hourly intervals](/metrics/details/frequency#hourly-frequency), and the `_1d` suffix on [daily intervals](/metrics/details/frequency#daily-frequency).
...
Each of the names above is also available with a `_1h` or `_1d` suffix — see [Frequency](#frequency).
